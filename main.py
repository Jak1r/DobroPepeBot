import os
import telebot
from telebot.types import InlineQueryResultArticle, InputTextMessageContent, InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup, KeyboardButton
...
# ========== ВРЕМЕННОЕ ХРАНИЛИЩЕ ==========
temp_images = {}
user_states = {}  # Для диалогов в личке
...
def send_pepe_wish(chat_id):
    ...
    try:
        ...
        if gif_files:
            ...
            # Если гифок нет — запасной вариант
            sent_message = bot.send_message(
                chat_id,
                "🎲 Кручу кубик..."
            )
        
        # 2. Через 2.5 секунды генерируем и отправляем картинку с пожеланием
        def edit_to_wish():
            time.sleep(2.5)
            try:
...
def inline_handler(inline_query):
    ...
    if query_text == "":
        ...
        if image_data:
            ...
            # Создаем результат с гифкой и автоматической заменой
